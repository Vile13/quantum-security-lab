...
Error bars are one standard deviation across seeds. They are drawn on every
series: a figure that shows means alone invites the reader to take differences
seriously that the spread does not support.
"""
...
def _rows_for(
    payload: dict, mechanism: str, layers: int
) -> tuple[list[float], list[float], list[float]]:
    """Strengths, mean accuracies and their standard deviations, sorted by strength."""
    rows = [
        r for r in payload["aggregate"]["evaluation"]
        if r["mechanism"] == mechanism and r["layers"] == layers
    ]
    rows.sort(key=lambda r: r["strength"])
    return (
        [r["strength"] for r in rows],
        [r["test_accuracy"]["mean"] for r in rows],
        [r["test_accuracy"]["std"] for r in rows],
    )


def plot_noise_sweeps(payload: dict, output_path: Path) -> Path:
    """One panel per mechanism, one line per layer count."""
    mechanisms = list(MECHANISM_TITLES)
    depths = payload["config"]["layer_counts"]
    n_seeds = len(payload["config"]["seeds"])
    ideal_by_depth = {
        r["layers"]: r["test_accuracy"]["mean"]
        for r in payload["aggregate"]["evaluation"] if r["mechanism"] == "none"
    }
    ...
    for ax, mechanism in zip(axes.flat, mechanisms, strict=True):
        plotted_any = False
        for depth in depths:
            strengths, accuracies, errors = _rows_for(payload, mechanism, depth)
            if strengths:
                ax.errorbar(strengths, accuracies, yerr=errors, marker="o", capsize=3,
                            linewidth=1.4, markersize=4, label=f"{depth} re-uploading layers")
                plotted_any = True
    ...
    fig.suptitle(
        ...
        f"noiseless reference -- {subtitle};  error bars = 1 SD over {n_seeds} seeds;  "
        "dotted line = chance level",
        fontsize=11,
    )
    fig.tight_layout()
...
def plot_depth_tradeoff(payload: dict, output_path: Path) -> Path:
    ...
    depths = payload["config"]["layer_counts"]
    n_seeds = len(payload["config"]["seeds"])
    rows = payload["aggregate"]["evaluation"]
    ideal, ideal_err, composite, composite_err, shift, shift_err = [], [], [], [], [], []
    for depth in depths:
        at_depth = [r for r in rows if r["layers"] == depth]
        none_row = next(r for r in at_depth if r["mechanism"] == "none")
        composite_row = next(r for r in at_depth if r["mechanism"] == "composite")
        ideal.append(none_row["test_accuracy"]["mean"])
        ideal_err.append(none_row["test_accuracy"]["std"])
        composite.append(composite_row["test_accuracy"]["mean"])
        composite_err.append(composite_row["test_accuracy"]["std"])
        shift.append(composite_row["mean_abs_proba_shift"]["mean"])
        shift_err.append(composite_row["mean_abs_proba_shift"]["std"])

    two_qubit_gates = {t["layers"]: t["two_qubit_gates"] for t in payload["aggregate"]["training"]}
    ...
    # near-identical at every depth, and a reader has to be able to see that
    # rather than assume one series failed to plot.
    ax.errorbar(x, ideal, yerr=ideal_err, marker="o", color="tab:blue", linewidth=5,
                alpha=0.35, capsize=4, label="accuracy, noiseless")
    ax.errorbar(x, composite, yerr=composite_err, marker="s", color="tab:green",
                linestyle="--", linewidth=1.5, markersize=5, capsize=3,
                label="accuracy, device-like noise")
    ax.axhline(CHANCE_LEVEL, color="grey", linestyle=":", linewidth=1)
    ax.set_xticks(x)
    ...
    twin = ax.twinx()
    twin.bar(x, shift, yerr=shift_err, width=0.35, color="tab:red", alpha=0.25, zorder=0,
             capsize=4, ecolor="tab:red", label="mean |probability shift|")
    twin.set_ylabel("mean |probability shift| vs. noiseless", color="tab:red")
    twin.tick_params(axis="y", labelcolor="tab:red")
    twin.set_ylim(0, (max(shift) + max(shift_err)) * 2.2 if shift else 1)
    ...
    ax.set_title(
        "Accuracy hides what the probabilities are already doing\n"
